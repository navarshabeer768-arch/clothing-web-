...
const navigation = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Trousers', href: '/shop/trousers-men' },
      { label: 'Outerwear', href: '/shop/outerwear-men' },
  ...
  },
  {
    ...
    href: '/shop/watches',
    children: [
      { label: 'Sport', href: '/shop/watches?style=sport' },
      { label: 'Dress', href: '/shop/watches?style=dress' },
      { label: 'Limited Edition', href: '/shop/watches?limited=true' },
      { label: 'View All', href: '/shop/watches' },
    ],
  },
  {
    label: 'Collections',
    ...
    <>
      ...
      <header
        className={cn(
          ...
          scrolled
            ? 'bg-white/95 backdrop-blur-sm shadow-[0_1px_20px_rgba(0,0,0,0.06)]'
            : 'bg-white'
        )}
      >
        <div className="container-luxury">
          <div className="flex items-center justify-between h-16 md:h-20">
                  ...
                  <AnimatePresence>
                    {item.children && activeMenu === item.label && (
                      <motion.div
                        ...
                        className="absolute top-full left-0 mt-2 w-48 bg-white shadow-luxury border border-luxury-100 rounded-sm"
                      >
                        {item.children.map((child, i) => (
                          <Link
                            ...
                            href={child.href}
                            className={cn(
                              'block px-5 py-3 text-xs tracking-widest uppercase font-medium text-foreground/70 hover:text-foreground hover:bg-luxury-50 transition-colors duration-150',
                              i < item.children!.length - 1 && 'border-b border-luxury-50',
                              i === item.children!.length - 1 && 'text-gold-500 hover:text-gold-600'
                            )}
                          >
                            {child.label}
                          </Link>
            ...
            <Link
              href="/"
              className="absolute left-1/2 -translate-x-1/2 text-2xl md:text-3xl font-light tracking-[0.3em]"
            ...
            <div className="flex items-center gap-1 md:gap-2">
              <button
                ...
                className="p-2.5 hover:bg-luxury-50 rounded-sm transition-colors duration-150"
                aria-label="Search"
              >
              ...
              <Link
                ...
                className="relative p-2.5 hover:bg-luxury-50 rounded-sm transition-colors duration-150"
                aria-label="Wishlist"
              >
              ...
              <Link
                href="/account"
                className="p-2.5 hover:bg-luxury-50 rounded-sm transition-colors duration-150 hidden md:block"
                aria-label="Account"
              >
          ...
          {mobileOpen && (
            ...
            >
              <nav className="container-luxury py-4 space-y-1">
                ...
                <Link
                  href="/account"
                  className="block py-3 px-2 text-xs tracking-widest uppercase font-medium text-foreground/70 hover:text-foreground"
                  onClick={() => setMobileOpen(false)}
                >
                  Account
                </Link>
          ...
          >
            <div className="container-luxury pt-24 pb-16">
              <button
                ...
                className="absolute top-6 right-6 p-2 hover:bg-luxury-50 rounded-sm"
              >
                <X className="h-5 w-5" />
              </button>
                ...
                <div className="mt-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['Cashmere Coat', 'Automatic Watch', 'Silk Dress', 'Leather Bag', 'Chronograph'].map((term) => (
                      <button
                        key={term}
                        className="px-4 py-2 text-xs tracking-wider uppercase border border-luxury-200 hover:border-luxury-400 hover:bg-luxury-50 transition-colors duration-150 rounded-sm"
                      >
                        {term}
                      </button>
                    ))}
