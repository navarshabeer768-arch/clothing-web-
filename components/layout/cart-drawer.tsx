          ...
          <motion.div
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-luxury-100">
              <div>
                ...
                {count > 0 && <p className="text-xs text-muted-foreground mt-0.5">{count} item{count !== 1 ? 's' : ''}</p>}
              </div>
              <button onClick={toggleCart} className="p-2 hover:bg-luxury-50 rounded-sm transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-16">
                  ...
                  <h3 className="text-lg font-light" style={{ fontFamily: 'var(--font-display)' }}>Your bag is empty</h3>
                  <p className="text-sm text-muted-foreground mt-2 mb-8">Discover our exclusive collection</p>
                  <button onClick={toggleCart} className="btn-luxury text-xs">Continue Shopping</button>
                </div>
              ) : (
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <motion.div key={item.id} layout initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                        exit={{ opacity: 0, x: -20 }} className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <Link href={`/shop/${item.product?.slug}`} onClick={toggleCart}
                              className="text-sm font-medium leading-tight hover:underline truncate block">
                              {item.product?.name}
                            </Link>
                            <button onClick={() => removeItem(item.id)} className="flex-shrink-0 p-1 hover:text-destructive transition-colors">
                          ...
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center border border-luxury-200">
                              <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1.5 hover:bg-luxury-50"><Minus className="h-3 w-3" /></button>
                              <span className="px-3 text-xs font-medium tabular-nums min-w-[2rem] text-center">{item.quantity}</span>
                              <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1.5 hover:bg-luxury-50"><Plus className="h-3 w-3" /></button>
                            </div>
            ...
            {items.length > 0 && (
              <div className="border-t border-luxury-100 px-6 py-6 space-y-4">
                ...
                <div className="space-y-2">
                  <Link href="/checkout" onClick={toggleCart} className="btn-luxury w-full text-center block">
                    Checkout · {formatPrice(total)}
                  </Link>
                  <Link href="/cart" onClick={toggleCart} className="btn-luxury-outline w-full text-center block">
                    View Bag
                  </Link>
                </div>
