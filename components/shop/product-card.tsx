...
export function ProductCard({ product, className }: ProductCardProps) {
  ...
  const discountPercent =
    product.compare_price ? getDiscountPercentage(product.base_price, product.compare_price) : null
    ...
    >
      ...
      <Link href={`/shop/${product.slug}`} className="block">
        <div className="relative aspect-[3/4] overflow-hidden bg-luxury-50 rounded-sm">
          ...
          <div
            className={cn(
              'absolute inset-x-0 bottom-0 p-4 flex flex-col gap-2 transition-all duration-300',
          ...
          <button
            ...
            className={cn(
              'absolute top-3 right-3 p-2 rounded-full bg-white/80 backdrop-blur-sm shadow-sm transition-all duration-200 z-10',
        ...
        <div className="mt-3 space-y-1">
          <h3 className="text-sm font-medium leading-tight group-hover:text-gold-600 transition-colors duration-150 line-clamp-1">
            {product.name}
          </h3>
          ...
          {product.review_stats && product.review_stats.review_count > 0 && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-[10px] text-muted-foreground ml-0.5">
                ({product.review_stats.review_count})
              </span>
            </div>
          )}
